...
async function getQuestions(type, userId) {
    const redisClient = getRedisClient();
    let conn = null;
    try {
        conn = await postgre.connect();
        let idxRange = type === 'dog' ? '1 AND 20' : '21 AND 40';
        const key = `questions:${type}:${userId}`;

        //질문을 찾는 쿼리문
        const { rows } = await postgre.query(
            `SELECT * FROM peti_question WHERE ${type} IS NOT NULL AND idx BETWEEN ${idxRange} ORDER BY RANDOM()`
        );
